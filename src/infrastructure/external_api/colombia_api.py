...
class ColombiaDatosAbiertosClient:
    ...
    def get_prescriptive_programs_data(self) -> pd.DataFrame:
        ...
        """
        # Intentamos obtener registros del dataset
        df = self.fetch_dataset("ddau-8cy9", limit=50)
        
        # Si el dataset remoto no responde o está vacío, estructuramos un fallback representativo
        if df.empty:
            df = pd.DataFrame([
